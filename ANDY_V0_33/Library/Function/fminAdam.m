function [x, fval, exitflag, output] = fminAdam(fun, x0, stepSize, beta1, beta2, epsilon, nEpochSize, options)

% fminAdam - FUNCTION Adam optimiser, with matlab calling format
%
% Usage: [x, fval, exitflag, output] = fminAdam(fun, x0 <, stepSize, beta1, beta2, epsilon, nEpochSize, options>)
%
% 'fminAdam' is an implementation of the Adam optimisation algorithm
...
if (nargin < 2)
   help fminAdam;
   error('*** fminAdam: Incorrect usage.');
end
...
if (isequal(options.GradObj, 'on'))
   ...
   if (nargout(fun) < 2) && (nargout(fun) ~= -1)
      error('*** fminAdam: The supplied cost function must return analytical gradients, if options.GradObj = ''on''.');
   end
...
if (options.UseParallel)
   warning('--- fminAdam: Warning: ''UseParallel'' is not yet implemented.');
end
...
if (~bUseAnalyticalGradients)
   ...
   if (max(abs(vfGrad0 - vfGrad1)) > eps(max(max(abs(vfGrad0), abs(vfGrad1)))))
      error('*** fminAdam: Cost function must return identical stochastic gradients for a given ''nIter'', when analytical gradients are not provided.');
   end
end
...
try
   ...
   error('*** fminAdam: Error when evaluating function to minimise.');
end


% - Check that initial point is reasonable
if (any(isinf(fval0)) || any(isnan(fval0)) || any(any(isinf(vfCdX0))) || any(any(isnan(vfCdX0))))
   error('*** fminAdam: Invalid starting point for user function. NaN or Inf returned.');
end
...
function FA_CheckGradients(fun, x0)
   ...
   disp('--- fminAdam: Checking analytical gradients...');
   ...
   if (fMaxDiff > fTolGrad)
      ...
      error('*** fminAdam: Gradient check failed.');
   end
