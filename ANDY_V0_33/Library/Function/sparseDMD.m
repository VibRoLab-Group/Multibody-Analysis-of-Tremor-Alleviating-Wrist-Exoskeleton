...
function answer = sparseDMD(P,q,s,gammaval,options)
...
Plow_star = Plow';
    ...
    for ii = 1:length(gammaval)
        ...
        % Record output data
        answer.xsp(:,ii) = z; % vector of amplitudes
        answer.Nz(ii) = nnz(answer.xsp(:,ii)); % number of non-zero amplitudes
        answer.Jsp(ii) = real(z'*P*z) - 2*real(q'*z) + s; % Frobenius norm (before polishing)
        ...
        answer.Jpol(ii) = real(xpol'*P*xpol) - 2*real(q'*xpol) + s; 
        % Polished (optimal) performance loss 
        answer.Ploss(ii) = 100*sqrt(answer.Jpol(ii)/s);

        fprintf("Gamma #"+num2str(ii)+"...\n")
    end     
